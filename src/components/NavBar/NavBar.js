...
import '../Config/ConfigMenus/People'
    ...
    <div className={`floating-menu scale-transition ${show ? 'scale-in': 'scale-out'}`}>
      <div>
        <div className="switch toggle-switch" style={{margin: "0", padding: "1rem"}}>
          <label>
            <input type="checkbox" defaultChecked={theme === 'dark'} onClick={handleThemeChange} disabled/>
      ...
      <div style={{minHeight: "0px !important", height: 'max-content', display: 'flex'}} >
          <h6 style={{height: '3rem', margin: 'auto'}}> No Signing Out</h6>
      </div>
    </div>
  )
}
...
const TeacherLinks = ({ showLogoMenu, setShowLogoMenu }) => {
  ...
  return (
    <div>
      ...
      <Link className="navbar-link" to="/config/general">
        Settings
      </Link>
    </div>
  )
